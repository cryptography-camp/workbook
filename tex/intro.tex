...
\begin{definition}[Hash function]
  ...
  \item $\algo{Eval}(\kappa, x) \rightarrow y$ is a deterministic algorithm that takes a hashing key $\kappa$ and an input $x \in \{0,1\}^*$ as input and outputs a hash value $y \in S$ where $S$ is a set with $|S| \geq 2^{c\lambda}$ for some integer $c > 0$.\footnote{For notational simplicity, $\algo{H.Eval}(\kappa, x)$ is often written as $\algo{H}(x)$, leaving the hashing key implicit. When not specified otherwise, we assume $S = \{0,1\}^{c\lambda}$.}
  \end{itemize}
\end{definition}
