...
\begin{definition}[Schnorr Signature with Key Prefixing]\label{def:schnorr-kp}
  ...
  \[
    c = \algo{H}(R \| X \| m)
  \]
  instead of $c = \algo{H}(R \| m)$, where $X$ is the public key.
\end{definition}
...
\ifsolutions
\begin{mysolution}
  A signature scheme $\algo{Sig} = (\algo{Setup}, \algo{KeyGen}, \algo{Sign}, \algo{Verify})$ with tweaking scheme $\algo{TwSch} = (\mathcal{T}, \algo{TwSK}, \algo{TwPK})$ is \emph{existentially unforgeable under chosen message attack with tweaking (EUF-CMA-TK)} if for all p.p.t. adversaries $\adv$:
  \[
...
\begin{exercise}
  Give a p.p.t. adversary that breaks the EUF-CMA-TK security of Schnorr signatures with the SchnorrTS tweaking scheme. Hints:
  \begin{itemize}
