...
\begin{definition}[Pedersen Commitment Scheme]
  The Pedersen commitment scheme $\algo{PedCom}[\grgen]$ with message space $\mathcal{M} = \ZZ_p$ is parameterized by a group generation algorithm $\grgen$ and defined as follows:
  \begin{itemize}
  \item $\algo{Setup}(1^\secpar) \rightarrow \params$: Run $\gparam \gets \grgen(1^\secpar)$, sample $h \sample \GG \setminus \{1_\GG\}$, and output $\params = (\gparam, h, \mathcal{R} \defeq \ZZ_p)$.
  \item $\algo{Commit}(\params, m; r) \rightarrow C$: Parse $\params = ((\GG, p, g), h, \mathcal{R})$. For message $m \in \ZZ_p$ and randomness $r \in \mathcal{R}$, output $C = g^m h^r$.
  \end{itemize}
\end{definition}
...
\begin{definition}[ElGamal Commitment Scheme]
  The ElGamal commitment scheme $\algo{ElGamalCom}[\grgen]$ with message space $\mathcal{M} = \ZZ_p$ is parameterized by a group generation algorithm $\grgen$ and defined as follows:
  \begin{itemize}
  \item $\algo{Setup}(1^\secpar) \rightarrow \params$: Run $\gparam \gets \grgen(1^\secpar)$, sample $h \sample \GG \setminus \{1_\GG\}$, and output $\params = (\gparam, h, \mathcal{R} \defeq \ZZ_p)$.
  \item $\algo{Commit}(\params, m; r) \rightarrow C$: Parse $\params = ((\GG, p, g), h, \mathcal{R})$. For message $m \in \ZZ_p$ and randomness $r \in \mathcal{R}$, output $C = (g^r, h^{m+r})$.
  \end{itemize}
\end{definition}
