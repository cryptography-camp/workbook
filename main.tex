...
\begin{proof}[Proof of Lemma~\ref{lem:negl}(2)]
  ...
  Let $q(\secpar)$ be an arbitrary polynomial, and let $a$ be a positive integer such that $q(\lambda) \le \lambda^a$ for sufficiently large $\lambda$.
  Since $f_1$ and $f_2$ are negligible, for the polynomial $\lambda^{a+1}$, there exist $N_1, N_2 \in \NN$ such that
  \[
  ...
  \[
    f_1(\secpar) + f_2(\secpar) \le \frac{1}{\lambda^{a+1}} + \frac{1}{\lambda^{a+1}} = \frac{2}{\lambda^{a+1}} \le \frac{1}{\lambda^{a}} \le \frac{1}{q(\secpar)}
  \]
\end{proof}
